...
class Command(BaseCommand):
    ...
    def create_routes(self):
        ...
        # Define popular routes with pincode data
        route_data = [
            # Mumbai-Delhi corridor
            {
                ...
                'vendor_index': 0,
                'stops': [
                    {'city': 'Nashik', 'lat': 19.9975, 'lng': 73.7898, 'pincode': '422001', 'order': 1},
                    {'city': 'Indore', 'lat': 22.7196, 'lng': 75.8577, 'pincode': '452001', 'order': 2},
                    {'city': 'Gwalior', 'lat': 26.2183, 'lng': 78.1828, 'pincode': '474001', 'order': 3}
                ]
            },
            {
                ...
                'vendor_index': 1,
                'stops': [
                    {'city': 'Jaipur', 'lat': 26.9124, 'lng': 75.7873, 'pincode': '302001', 'order': 1},
                    {'city': 'Udaipur', 'lat': 24.5854, 'lng': 73.7125, 'pincode': '313001', 'order': 2},
                    {'city': 'Ahmedabad', 'lat': 23.0225, 'lng': 72.5714, 'pincode': '380001', 'order': 3}
                ]
            },
            # Bangalore-Chennai corridor
            {
                ...
                'vendor_index': 2,
                'stops': [
                    {'city': 'Hosur', 'lat': 12.7409, 'lng': 77.8253, 'pincode': '635109', 'order': 1},
                    {'city': 'Krishnagiri', 'lat': 12.5266, 'lng': 78.2150, 'pincode': '635001', 'order': 2}
                ]
            },
            {
                ...
                'vendor_index': 3,
                'stops': [
                    {'city': 'Vellore', 'lat': 12.9165, 'lng': 79.1325, 'pincode': '632001', 'order': 1},
                    {'city': 'Hosur', 'lat': 12.7409, 'lng': 77.8253, 'pincode': '635109', 'order': 2}
                ]
            },
            # Mumbai-Pune corridor
            {
                ...
                'vendor_index': 0,
                'stops': [
                    {'city': 'Lonavala', 'lat': 18.7537, 'lng': 73.4068, 'pincode': '410401', 'order': 1}
                ]
            },
            # Delhi-Kolkata corridor
            {
                ...
                'vendor_index': 4,
                'stops': [
                    {'city': 'Kanpur', 'lat': 26.4499, 'lng': 80.3319, 'pincode': '208001', 'order': 1},
                    {'city': 'Allahabad', 'lat': 25.4358, 'lng': 81.8463, 'pincode': '211001', 'order': 2},
                    {'city': 'Varanasi', 'lat': 25.3176, 'lng': 82.9739, 'pincode': '221001', 'order': 3}
                ]
            },
            # Hyderabad routes
            {
                ...
                'vendor_index': 2,
                'stops': [
                    {'city': 'Kurnool', 'lat': 15.8281, 'lng': 78.0373, 'pincode': '518001', 'order': 1},
                    {'city': 'Tirupati', 'lat': 13.6288, 'lng': 79.4192, 'pincode': '517001', 'order': 2}
                ]
            },
            # Bangalore-Mumbai corridor
            {
                ...
                'vendor_index': 2,
                'stops': [
                    {'city': 'Hubli', 'lat': 15.3647, 'lng': 75.1240, 'pincode': '580001', 'order': 1},
                    {'city': 'Belgaum', 'lat': 15.8497, 'lng': 74.4977, 'pincode': '590001', 'order': 2},
                    {'city': 'Kolhapur', 'lat': 16.7050, 'lng': 74.2433, 'pincode': '416001', 'order': 3},
                    {'city': 'Pune', 'lat': 18.5204, 'lng': 73.8567, 'pincode': '411001', 'order': 4}
                ]
            }
        ]
        ...
        for route_info in route_data:
            ...
            dlon = math.radians(dest_coords['lng'] - origin_coords['lng'])
            a = math.sin(dlat/2) * math.sin(dlat/2) + math.cos(math.radians(origin_coords['lat'])) \
                * math.cos(math.radians(dest_coords['lat'])) * math.sin(dlon/2) * math.sin(dlon/2)
            ...
            # Map cities to primary pincodes
            city_to_pincode = {
                'Mumbai': '400001',
                'Delhi': '110001',
                'Bangalore': '560001',
            ...
            route, created = Route.objects.get_or_create(
                ...
                defaults={
                    ...
                    'origin_state': city_to_state.get(route_info['origin'], 'Unknown'),
                    'origin_pincode': route_info.get('origin_pincode', city_to_pincode.get(route_info['origin'], '')),
                    'destination_city': route_info['destination'], 
                    'destination_state': city_to_state.get(route_info['destination'], 'Unknown'),
                    'destination_pincode': route_info.get('dest_pincode', city_to_pincode.get(route_info['destination'], '')),
            ...
            if created:
                ...
                for stop_info in route_info.get('stops', []):
                    ...
                    stop_lat, stop_lng = stop_info['lat'], stop_info['lng']
                    
                    # Distance from origin
                    dlat_origin = math.radians(stop_lat - origin_coords['lat'])
                    dlon_origin = math.radians(stop_lng - origin_coords['lng'])
                    a_origin = math.sin(dlat_origin/2) * math.sin(dlat_origin/2) + math.cos(math.radians(origin_coords['lat'])) \
                        * math.cos(math.radians(stop_lat)) * math.sin(dlon_origin/2) * math.sin(dlon_origin/2)
                    c_origin = 2 * math.atan2(math.sqrt(a_origin), math.sqrt(1-a_origin))
                    distance_from_origin = R * c_origin
                    
                    # Distance to destination
                    dlat_dest = math.radians(dest_coords['lat'] - stop_lat)
                    dlon_dest = math.radians(dest_coords['lng'] - stop_lng)
                    a_dest = math.sin(dlat_dest/2) * math.sin(dlat_dest/2) + math.cos(math.radians(stop_lat)) \
                        * math.cos(math.radians(dest_coords['lat'])) * math.sin(dlon_dest/2) * math.sin(dlon_dest/2)
                    c_dest = 2 * math.atan2(math.sqrt(a_dest), math.sqrt(1-a_dest))
                    distance_to_destination = R * c_dest
                    
                    RouteStop.objects.create(
                        route=route,
                        stop_city=stop_info['city'],
                        stop_state=city_to_state.get(stop_info['city'], 'Unknown'),
                        stop_pincode=stop_info.get('pincode', city_to_pincode.get(stop_info['city'], '')),
                        stop_latitude=Decimal(str(stop_info['lat'])),
                        stop_longitude=Decimal(str(stop_info['lng'])),
                        stop_order=stop_info['order'],
                        distance_from_origin=Decimal(str(round(distance_from_origin, 2))),
                        distance_to_destination=Decimal(str(round(distance_to_destination, 2))),
                        can_pickup=True,
                        can_drop=True,
    ...
    def create_route_pricing(self, route, truck_types):
        ...
        for truck_type in truck_types:
            ...
            fuel_charges = Decimal(str(distance * 2.5))
            toll_charges = Decimal(str(max(50, distance * 1.5)))
            loading_charges = Decimal('200')
            unloading_charges = Decimal('200')
            
            # Calculate total price and set min/max ranges
            total_price = base_price + fuel_charges + toll_charges + loading_charges + unloading_charges
            min_price = total_price * Decimal('0.9')  # 10% lower
            max_price = total_price * Decimal('1.2')  # 20% higher
            ...
            RoutePricing.objects.create(
                ...
                segment_distance_km=Decimal(str(round(distance, 2))),
                ...
                available_vehicles=random.randint(1, 5),
                is_active=True
            )
